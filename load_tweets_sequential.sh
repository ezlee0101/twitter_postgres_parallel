#!/bin/bash

files=$(find data/*)

echo '================================================================================'
echo 'load denormalized'
echo '================================================================================'
time for file in $files; do
    echo "→ Loading $file into pg_denormalized"
    unzip -p "$file" \
    | sed 's/\\u0000//g' \
    | psql postgresql://postgres:pass@localhost:4115/postgres \
        -c "COPY tweets_jsonb (data) FROM STDIN CSV QUOTE E'\\x01' DELIMITER E'\\x02';"
    # copy your solution to the twitter_postgres assignment here
done

echo '================================================================================'
echo 'load pg_normalized'
echo '================================================================================'
time for file in $files; do
    echo "→ Loading $file into pg_normalized"
    python3 load_tweets.py \
    --db postgresql://postgres:pass@localhost:4116/postgres \
    --inputs "$file" \
    --print_every 10000
    # copy your solution to the twitter_postgres assignment here
done

echo '================================================================================'
echo 'load pg_normalized_batch'
echo '================================================================================'
time for file in $files; do
    echo "→ Loading $file into pg_normalized_batch"
    python3 -u load_tweets_batch.py \
    --db postgresql://postgres:pass@localhost:4537/ \
    --inputs "$file"
done
